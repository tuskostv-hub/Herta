...
| **C++20 modules в самом компиляторе** вместо заголовочных файлов | **Да** — все исходники `.cppm`, `import std;`, никаких `.hpp` в проекте |
| IR + оптимизации (constant folding)                 | **Да** — отдельная фаза между semantic и codegen (см. «Что дальше») |
| Полноценная межмодульная сборка Herta-кода          | Нет — синтаксис `module`/`import`/`pub` парсится, но компилятор работает только с одним `.herta`-файлом |

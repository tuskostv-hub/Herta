...
| **A.1.6** | Автоматический вывод типов переменных (`:=`) | **Готово** |
| **A.1.7** | Неявные приведения числовых типов (widening) | **Готово** — `types.md §5.4` |
| **A.2.3** | Методы структур (`impl`) | **Готово** — instance методы `obj.method(args)`, static методы `T.method(args)`, dispatch по типу, проверка self |
| **A.2.13** | Модули | **Готово** — `module`/`import`/`pub`, driver рекурсивно загружает `.herta`-файлы, цикл-детект через DFS, проверка module ↔ filename |
...
| **B.2.1** | Промежуточное представление (IR) | Запланировано |
| **B.2.2** | Оптимизации IR (constant folding) | Запланировано |
...
CLI:
```
myc <source.herta> [-o <output>] [--dump-tokens] [--dump-ast] [--dump-ir]
```

...
## Реализуемые «Доп» из ТЗ v2.0

| Код  | Описание | Статус |
|------|----------|--------|
| **A.1.1** | Целые от 8 до 64 бит со знаком и без | **Готово** |
| **A.1.2** | UTF-8 в строках и символах | **Готово** — char-литералы поддерживают 1-4 байта UTF-8, декодируются в codepoint |
| **A.1.7** | Автоматический вывод типов переменных (`:=`) | **Готово** |
| **A.1.8** | Неявные приведения числовых типов (widening) | **Готово** — `types.md §5.4` |
| **A.2.12** | Методы структур и видимость членов (`impl`) | **Готово** — instance методы `obj.method(args)`, static методы `T.method(args)`, dispatch по типу, проверка self |
| **A.2.18** | Указатели | **Готово** — `*T`, `&x`, `*p` (rvalue/lvalue), `null` + runtime-проверка, тип `byte` |
| **A.2.20** | Модули | **Готово** — `module`/`import`/`pub`, driver рекурсивно загружает `.herta`-файлы, цикл-детект через DFS, проверка module ↔ filename |
| **B.2.1** | Промежуточное представление (IR) | **Готово** — линейный трёхадресный код, `--dump-ir` |
| **A.3.9** | Обобщённые и сырые указатели | **Готово** (частично) — `*void`/`*byte` raw, type `byte` (non-arithmetic), приведения указателей; **function pointers — нет** (диагностика)
| **A.3.14** | Модульная система с контролем видимости | **Готово** — `pub` строго разграничивает экспорт; приватные имена недоступны через `M.x` |
...
Pipeline (однофайловая компиляция):
...
```
   ...
   [Parser]       → AST
        ↓
   [Semantic]     → проверки и аннотированный AST
        ↓
...
- **Этап 6 — кодогенерация.** ✓ Готов. Путь **LLVM IR + clang**: модуль `herta.llvm` печатает текстовый LLVM IR, `myc` зовёт `clang-22` для сборки нативного x86-64 ELF; runtime — `src/codegen/runtime.c`. Подробности — в `specs/codegen.md`.

CLI:
```
myc <source.herta> [-o <out>] [--run] [--emit-llvm] [--dump-tokens] [--dump-ast] [--dump-ir]
```
Один входной файл — один результат.

Каждый этап будет детализирован по мере завершения предыдущего.
